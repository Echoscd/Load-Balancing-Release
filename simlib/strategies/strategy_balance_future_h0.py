...
# -------- Tunable parameters --------
EPS = 1e-6
HORIZON_MAX = 0                  # H=0: only consider current load, no future prediction
...
def compute_lambda_from_loads(sim, loads_mat, tol: float = 1e-6) -> float:
    ...
    maxv = loads_mat[1:].max(axis=1) if loads_mat.shape[0] > 1 else np.array([])
    if maxv.size == 0:
        rem_steps = 0
    else:
...
def build_F_evaluator_fast(
    ...
    αD = ALPHA * prefix_d[-1]
    m = int(np.floor(float(ms[0]))) if ms.size > 0 else 0
...
# ========= Main strategy =========
def policy_balance_future_h0(sim) -> None:
    """
    Balance Future H=0 strategy: only consider current load, no future prediction.
    Two-phase approach:
      1) Greedy single-request phase: When remaining slots are abundant, repeatedly select GPU with
         ...
         that maximizes F(s), assign and incrementally update current load.
      2) Fine-grained phase: Build heap + subset DP, batch assignment under current load constraints.
    """
    ...
    # 1) Predict future load (H=0 only considers current load)
    ...
    while sim.wait_q:
        ...
        # Incrementally update this GPU's load and global envelope
